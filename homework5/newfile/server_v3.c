		...
		if (FD_ISSET(sock_2, &readfds)){
			...
			if (n < 0){
				...
				if (buffer[0] == 'S' && buffer[1] == 'E'){
					...
					char *message = malloc((msglen + 1)*sizeof(char));
				...
				if (buffer[0] == 'B' && buffer[1] == 'R'){
					...
					char *message2 = malloc((msglen2 + 1) * sizeof(char));
		...
		if (n == -1){
			...
			if (buffer[0] == 'L' && buffer[1] == 'O' && buffer[2] == 'G' && buffer[3] == 'I' && buffer[4] == 'N'){
				...
				*/
				...
				for (i = 0; i < index_g; i++){
					if (addr_list[i].sin_addr.s_addr == client.sin_addr.s_addr && addr_list[i].sin_port == client.sin_port && socket_list[i] == new_sock){
						if (server_type[i] == 2){
							send(new_sock, "OK\n", 3, 0);
							char * tmp = malloc((strlen(namebuffer) + 1)*sizeof(char));
							strcpy(tmp, namebuffer);
			...
			if (buffer[0] == 'W' && buffer[1] == 'H'){
				...
				if (mark == 0){
					send(new_sock, "ERROR Not logged in\n", 20, 0);
					continue;
				}
			...
			if (buffer[0] == 'S' && buffer[1] == 'E'){
				...
				char *message = malloc((1 + msglen)*sizeof(char));
				char *tmp_send = message;
				i = i + 1;
				while(buffer[i] != '\n'){
					*tmp_send = buffer[i];
			...
			if (buffer[0] == 'S' && buffer[1] == 'H'){
				...
				m = 0;
				char rst3[2000];
				m = sprintf(rst3, "SHARE ");
				index_send3 += m;
				m = sprintf(rst3 + index_send3, "%s", sender_name3);
				index_send3 += m;
				m = sprintf(rst3 + index_send3, " %d", msglen3);
				index_send3 += m;

				sendto(socket_list[target2], rst3, index_send3, 0, (struct sockaddr *)&addr_list[target2], sizeof(addr_list[target2]));
				
				while(1){
					n = recv(new_sock, share_buffer, 1024, 0);
					send(new_sock, "OK\n", 3, 0);
					if (n < 1024){
						sendto(socket_list[target2], share_buffer, n, 0, (struct sockaddr *)&addr_list[target2], sizeof(addr_list[target2]));
						break;
					}else{
