...
def Cal(flag): # calculate the related time
...
t_slice = 80 # the time slice in RR
# time 
...
def CPU_run(p_vec_ori, p_vec, r_queue, IO_q, flag, flag2):
	...
	print "time " + str(t_ec) + 'ms: Simulator ended for ' + flag2
	timeflies(cur_run, p_vec_ori, p_vec, r_queue, IO_q, flag, t_cs/2)

def LetsRock(flag2):
	#arguments
	print
	print "time " + str(t_ec) + 'ms: Simulator started for ' + flag2 + ' [Q <empty>]'
	...
	if  len(sys.argv) >= 4:
			flag = sys.argv[3]
	CPU_run(prc_vec_ori, process_vec, ready_queue, IO_q, flag, flag2 )
	Cal(flag2)
